		...
		try:
			if(productObj.productStock>=_qty):																#Check enough stock is available
				...
				if newBasket.basketID:																		#Check basket was properly saved
					...
					return JsonResponse({"success":False}, status=500)										#Return 500 - Server error - Basket was not created
			else:	
...
def setCookie(request):
	...
	response = JsonResponse({"success":False})									#default response body
	...
	try:
		...
		user_hashVal = body['user_hashVal']										#check data is correctly formatted
	except:
		return JsonResponse(response)											#data is not correctly formatted, inform client of failure
